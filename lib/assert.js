...
exports.object = (node, key1, interface) => {
  if (typeof node[key1] !== "object" || node[key1] === null) {
    throw error(`${node.type}.${key1} must be an non-null object`);
  }
  for (let key2 of interface) {
    if (typeof node[key1][key2] !== interface[key2]) {
      throw error(`${node.type}.${key1}.${key2} must be a ${interface[key2]}`);
    }
  }
};
