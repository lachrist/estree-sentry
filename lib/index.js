...
const raise_label_continue_empty = Token.raise(
  {
    __proto__: null,
    [Token.KIND_LABEL_CONTINUE]: `Empty continue label must be bound to a loop statement`},
  null);

const raise_marker_pattern_member = Token.raise(
  {
    __proto__: null,
    [Token.KIND_MEMBER_PATTERN]: `MemberExpression cannot be used as a pattern in a declaration context`});
...
const raise_garbage_program = Token.raise(
  {
    ...
    // Strict //
    [Token.KIND_MARKER_WITH]: raise_garbage_unexpected,
    [Token.KIND_MARKER_PATTERN_EVAL]: raise_garbage_unexpected,
    [Token.KIND_MARKER_PATTERN_ARGUMENTS]: raise_garbage_unexpected,
    [Token.KIND_MARKER_DISCARD]: raise_garbage_unexpected,
    [Token.KIND_MARKER_PARAM_DUPLICATE]: raise_garbage_unexpected});
...
const visitors = {
  ...
  ExportDefaultDeclaration: (node, context) => visit(
    node.declaration,
    "anonymous",
    "FunctionDeclaration",
    "ClassDeclaration",
