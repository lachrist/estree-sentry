...
const unexpected_token = (kind, data) => { throw new global_Error(`Unexpected token ${kind} carrying ${data}`) };
...
const test_variable_program = Token.test(
  {
    ...
    [Token.KIND_VARIABLE_VAR_DEEP]: null,
    [Token.KIND_VARIABLE_FUNCTION_DEEP]: null});
...
const raise_duplicate_variable_enum_rigid = {
  __proto__: null,
  [Token.KIND_VARIABLE_GENERIC_RIGID]: null,
  [Token.KIND_VARIABLE_FUNCTION_SHALLOW_RIGID]: null,
  [Token.KIND_VARIABLE_LET]: null,
  [Token.KIND_VARIABLE_CONST]: null,
  [Token.KIND_VARIABLE_CLASS]: null};
const raise_duplicate_variable_enum_loose = {
  ...
  [Token.KIND_VARIABLE_PARAM]: null,
  [Token.KIND_VARIABLE_GENERIC_LOOSE]: null,
  [Token.KIND_VARIABLE_FUNCTION_SHALLOW_LOOSE]: null,
  [Token.KIND_VARIABLE_VAR_SHALLOW]: null};
...
const transform_variable_qualify = {
  ...
  ["var"]: Token.transform(
    {
      __proto__: null,
      [Token.KIND_VARIABLE_VOID]: Token.KIND_VARIABLE_VAR_SHALLOW}),
  ["function"]: Token.transform(
    {
      __proto__: null,
      [Token.KIND_VARIABLE_VOID]: Token.KIND_VARIABLE_FUNCTION_SHALLOW_RIGID}),
  ["let"]: Token.transform(
    {
      __proto__: null,
...
const transform_variable_burrow = Token.transform(
  {
    __proto__: null,
    [Token.KIND_VARIABLE_VAR_SHALLOW]: Token.KIND_VARIABLE_VAR_DEEP,
    [Token.KIND_VARIABLE_FUNCTION_SHALLOW_LOOSE]: Token.KIND_VARIABLE_FUNCTION_DEEP,
    [Token.KIND_VARIABLE_FUNCTION_SHALLOW_RIGID]: Token.KIND_VARIABLE_FUNCTION_DEEP});

const transform_variable_function_loosen = Token.transform(
  {
    __proto__: null,
    [Token.KIND_VARIABLE_FUNCTION_SHALLOW_RIGID]: Token.KIND_VARIABLE_FUNCTION_SHALLOW_LOOSE});
...
const set_hoisting = (tokens, tag, node) => (
  ...
  tokens = ArrayLite.map(tokens, transform_variable_burrow),
...
const helper_for = (node) => (
  ...
  ArrayLite.concat(
    ...
    Token.check(
      ...
      `${node.type}.body cannot be a FunctionDeclaration`),
    Token.check(
      node.body.type !== "ClassDeclaration",
      `${node.type}.body cannot be a ClassDeclaration`),
    Token.check(
      (
        node.body.type !== "VariableDeclaration" ||
        node.body.kind === "var"),
      `${node.type}.body cannot be a let/const VariableDeclaration`),
    Token.guard(
      (
        node.type === "ForOfStatement" &&
        node.await),
...
const helper_function = (node, context, _tokens, _use_strict) => (
  ...
  (
    node.id === null ?
    null :
    Assert.node(node.id, ["Identifier"])),
  _tokens = visit(
    node.body,
    {
      __proto__: null,
...
const visitors = {
  ...
  ExportNamedDeclaration: (node, context) => (
    ...
    ArrayLite.concat(
      ...
      ArrayLite.flatMap(
        node.specifiers,
        (node) => visit(node, node.source === null ? null : "key", "ExportSpecifier")),
      (
        node.source === null ?
        [] :
        visit(node.source, null, "Literal")),
  ...
  IfStatement: (node, context) => ArrayLite.concat(
    ...
    Token.guard(
      node.consequent.type === "FunctionDeclaration",
      Token.KIND_STRICT,
      `In strict mode, IfStatement.consequent must not be a FunctionDeclaration`),
    Token.check(
      node.consequent.type !== "ClassDeclaration",
      `IfStatement.consequent cannot be a ClassDeclaration`),
    Token.check(
      (
        node.consequent.type !== "VariableDeclaration" ||
        node.consequent.kind === "var"),
      `IfStatement.consequent cannot be a let/const VariableDeclaration`),
    Token.guard(
      (
        node.alternate !== null &&
        node.alternate.type === "FunctionDeclaration"),
      Token.KIND_STRICT,
      `In strict mode, IfStatement.alternate must not be a FunctionDeclaration`),
    Token.check(
      (
        node.alternate === null ||
        node.alternate.type !== "ClassDeclaration"),
      `IfStatement.alternate cannot be a ClassDeclaration`),
    Token.check(
      (
        node.alternate === null ||
        node.alternate.type !== "VariableDeclaration" ||
        node.alternate.kind === "var"),
      `IfStatement.alternate cannot be a let/const VariableDeclaration`)),
  ...
  WhileStatement: (node, context) => ArrayLite.concat(
    ...
    Token.check(
      node.body.type !== "FunctionDeclaration",
      `WhileStatement.body cannot be a FunctionDeclaration`),
    Token.check(
      node.body.type !== "ClassDeclaration",
      `WhileStatement.body cannot be a ClassDeclaration`),
    Token.check(
      (
        node.body.type !== "VariableDeclaration" ||
        node.body.kind === "var"),
      `WhileStatement.body cannot be a let/const VariableDeclaration`)),
  DoWhileStatement: (node, context) => ArrayLite.concat(
    ...
    Token.check(
      node.body.type !== "FunctionDeclaration",
      `DoWhileStatement.body cannot be a FunctionDeclaration`),
    Token.check(
      node.body.type !== "ClassDeclaration",
      `DoWhileStatement.body cannot be a ClassDeclaration`),
    Token.check(
      (
        node.body.type !== "VariableDeclaration" ||
        node.body.kind === "var"),
      `DoWhileStatement.body cannot be a let/const VariableDeclaration`)),
  ForStatement: (node, context) => ArrayLite.concat(
    ...
    Token.check(
      node.body.type !== "FunctionDeclaration",
      `ForStatement.body cannot be a FunctionDeclaration`),
    Token.check(
      node.body.type !== "ClassDeclaration",
      `ForStatement.body cannot be a ClassDeclaration`),
    Token.check(
      (
        node.body.type !== "VariableDeclaration" ||
        node.body.kind === "var"),
      `ForStatement.body cannot be a let/const VariableDeclaration`)),
