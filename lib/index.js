...
const test_variable_closure = Token.test(
  {
    __proto__: null,
    [Token.KIND_VARAIBLE_PARAM]: null,
...
const helper_function = (node, context, _tokens) => (
  ...
  _tokens = set_capture(_tokens, test_variable_closure, node),
...
const visitors = {
  ...
  Program: (node, context, _tokens) => (
    ...
    _tokens = set_capture(
      _tokens,
      _strict ? options["capture-strict"] : options["capture-normal"],
      node),
    _tokens = ArryLite.concat(
      ArrayLite.map(options["scope-frame"], Token.from_variable),
      _tokens),
    _tokens = ArrayLite.map(_tokens, raise_variable_rigid_duplicate),
    _tokens = set_release(_tokens, node),
