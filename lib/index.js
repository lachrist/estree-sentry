...
const test_variable_non_param = Token.test(
  {
    __proto__: null,
    [Token.KIND_VARIABLE_LET]: null,
    [Token.KIND_VARIABLE_CONST]: null,
...
const test_variable_non_generic = Token.test(
  {
    ...
    [Token.KIND_VARIABLE_VAR]: null,
    [Token.KIND_VARIABLE_FUNCTION]: null,
    [Tokne.KIND_VARIABLE_PARAM]: null});
...
const hoist = (tokens, test, node) => (
  node[KEY_CAPTURE] = ArrayLite.map(
    ArrayLite.filter(tokens, test),
    Token.to_variable),
  tokens = ArrayLite.filterOut(tokens, test),
  node[KEY_RELEASE] = ArrayLite.map(
    ArrayLite.filter(tokens, test_variable),
    Token.to_variable),
  tokens);
...
exports.module = (node, options) => ArrayLite.map(
  visit(
    ...
    global_Object_assign(
      ...
      {
        __proto__: null,
        "capture-strict": test_variable_block_and_closure,
        "capture-normal": null,
...
const visitors = {
  ...
  Program: (node, context, _tokens) => (
    ...
    _tokens = ArrayLite.flatMap(
      ...
      (node) => visit(
        ...
        Estree.ENUM_TYPE_STATEMENT)),
    // Eval //
    node[KEY_EVAL] = ArrayLite.includes(_tokens, test_marker_eval),
    _tokens = ArrayLite.filterOut(_tokens, test_marker_eval),
    // Strict //
    node[KEY_STRICT] = Estree.has_use_strict(node.body, 0),
    (
      (node[KEY_STRICT] || options["strict-mode"]) ?
      _tokens = ArrayLite.filterOut(_tokens, test_marker_strict) :
      null),
    // Hoisting //
    _tokens = ArrayLite.map(_tokens, raise_variable_duplicate),
    _tokens = capture(
      _tokens,
      (
        (options["strict-mode"] || node[KEY_STRICT]) ?
        options["capture-strict"] :
        options["capture-normal"]),
      node),
    _tokens = ArryLite.concat(
      ArrayLite.map(options["scope-frame"], Token.from_generic_variable),
      _tokens),
    _tokens = ArrayLite.map(_tokens, raise_variable_duplicate),
    _tokens = release(_tokens, node),
    _tokens = ArrayLite.filterOut(_tokens, test_variable_non_param),
    // Module //
    (
      options["module"] ?
    ...
    (
      ...
      _tokens = ArrayLite.filterOut(_tokens, test_marker_super_call) :
      null),
    (
      (
        options["closure-tag"] == "derived-constructor" ||
      ...
      _tokens = ArrayLite.filterOut(_tokens, test_marker_super_member) :
      null),
    // Garbage //
  ...
  CatchClause: (node, _tokens) => release(
    capture(
      ArrayLite.map(
        visit(
          node
